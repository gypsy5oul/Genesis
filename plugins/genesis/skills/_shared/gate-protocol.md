...
5. STOP. Never invoke the next stage. Never edit `status` to `approved` — only the human's approval (hook) does that. This is enforced, not just documented: a PreToolUse hook (`sdlc-state-guard.js`) denies any direct `Edit`/`Write`/`MultiEdit` to `state.json` that would set a stage's `status` to `"approved"`, and heuristically denies common `Bash`-based tampering patterns too (e.g. `sed -i` mentioning both `state.json` and `approved`). This is a best-effort net, not an absolute guarantee — Bash is Turing-complete, so a sufficiently determined bypass isn't impossible — but it blocks direct and common-pattern tampering, consistent with any local-file permission model.
