...
- The statusline (opt-in, `~/.claude/settings.json`) shows live session + rolling-7-day token usage and an estimated USD cost — a SessionStart nudge offers to set it up until you do, and Genesis never overwrites an existing statusline without asking.
